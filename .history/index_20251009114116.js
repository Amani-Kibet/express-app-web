...
app.post("/inbox", async (req, res) => {
  ...
  const inbox = {};
  msgs.forEach(msg => {
    const otherUser = msg.sender === phone ? msg.receiver : msg.sender;
    ...
    if (new Date(msg.time) > new Date(inbox[otherUser].lastTime)) {
      inbox[otherUser].lastMessage = msg.message;
      inbox[otherUser].lastTime = msg.time;
    }

    // Count unread messages only if user is receiver
    if (msg.receiver === phone && !msg.read) {
      inbox[otherUser].unreadCount++;
    }
  });
  ...
  const result = await Promise.all(
    Object.values(inbox).map(async item => {
      const user = await contacts.findOne({ phone: item.user });
      return {
        phone: item.user,
